...
export const sendSubscriptionRenewalEmail = async (user: User, plan: Plan) => {
  const html = `
    <div style="font-family: Arial, sans-serif; padding: 20px; background-color: #f9f9f9;">
      <h2 style="color: #333;">¡Suscripción renovada!</h2>
      <p>Hola ${user.first_name} ${user.last_name},</p>
      <p>Has renovado con éxito el plan <strong>${plan.name}</strong>.</p>
  ...
  await sendEmail({
    to: user.email,
    subject: `Renovación de suscripción al plan ${plan.name}`,
    html,
  });
};
